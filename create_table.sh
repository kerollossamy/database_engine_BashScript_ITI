...
if [[ ! "$table_name" =~ ^[0-9_] ]]; then
    if [[ "$table_name" =~ ^[a-zA-Z][a-zA-Z0-9_[:space:]]*$ ]]; then
        ...
        if [ ${#table_name} -ge 2 ] && [ ${#table_name} -le 30 ]; then
            ...
            echo -e "\e[91mError: Table name should be at least 2 characters and not more than 30.\e[0m"
            cd ../../
            source table_menu.sh
        fi
    else
...
if [[ "$columns_num" =~ ^[0-9]+$ ]]; then
    for ((i = 2; i <= $columns_num + 1; i++)); do
        while true; do
            ...
            if [ -z "$column_name" ]; then
                ...
                echo -e "\e[91mError: column name should be more than 1 character.\e[0m"
            elif [[ ! "$column_name" =~ ^[a-zA-Z][a-zA-Z0-9_]*$ ]]; then
                echo -e "\e[91mError: Column name should:
      -start with a letter
      -can't contain spaces
      -can't start with numbers\e[0m"
            elif [[ ${#column_name} -gt 20 ]]; then
                echo -e "\e[91mError: column name shouldn't be more than 20 character.\e[0m"
            else
                break
            fi
        done
    ...
    echo -e "\e[94m------------------------------------------------------------\e[0m"
    echo -e "\e[92mTable metadata added to =>\e[0m \e[93m$table_name\e[0m \e[92m<= table.\e[0m"
    echo -e "\e[94m------------------------------------------------------------\e[0m"
else
    if [[ ! "$columns_num" =~ ^[0-9]+$ ]]; then
        echo -e "\e[93mInvalid input Please enter a numeric value for the number of columns.\e[0m"
        rm "$table_name"
